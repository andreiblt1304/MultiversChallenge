...
use multiversx_sc::{
    codec::multi_types::{MultiValue2, MultiValue3},
...
use subscription::{
    self,
    payments::{payments::PaymentsModule, substract_payments::SubtractPaymentsModule},
    ...
    SubscriptionObjBuilder: 'static + Copy + Fn() -> subscription::ContractObj<DebugApi>,
{
    pub fn new(
        ...
        b_mock
            .borrow_mut()
            .execute_tx(owner_address, &sub_wrapper, &rust_zero, |sc| {
    ...
    pub fn call_register_service(
        ...
        self.b_mock
            .borrow_mut()
            .execute_tx(caller, &self.sub_wrapper, &rust_biguint!(0), |sc| {
                let mut args_encoded = MultiValueEncoded::<
                    DebugApi,
                    MultiValue3<
                        ManagedAddress<DebugApi>,
                        Option<EgldOrEsdtTokenIdentifier<DebugApi>>,
                        BigUint<DebugApi>,
                    >,
                >::new();
                for arg in args {
                    let (sc_address, opt_token_id, value) = arg;
                    args_encoded.push(MultiValue3((
    ...
    pub fn call_deposit(&self, caller: &Address, token_id: &[u8], amount: u64) -> TxResult {
    ...
    pub fn call_subscribe(
        ...
        self.b_mock
            .borrow_mut()
            .execute_tx(caller, &self.sub_wrapper, &rust_biguint!(0), |sc| {
                let mut managed_services = MultiValueEncoded::<
                    DebugApi,
                    MultiValue3<AddressId, usize, SubscriptionType>,
                >::new();
                for arg in args {
    ...
    pub fn call_withdraw_funds(
        ...
        self.b_mock
            .borrow_mut()
            .execute_tx(caller, &self.sub_wrapper, &rust_biguint!(0), |sc| {
                let mut tokens_to_withdraw = MultiValueEncoded::<
                    DebugApi,
                    MultiValue2<EgldOrEsdtTokenIdentifier<DebugApi>, BigUint<DebugApi>>,
                >::new();
                for token in tokens {
